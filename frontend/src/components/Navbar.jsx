  ...
  useEffect(() => {
    if (user) {
      const onConnect = () => {
        ...
        console.log('[Socket] Global connect: Joining room', user.id);
      };
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            {user?.role === 'user' && (
              <NotificationCenter 
                socket={socket} 
                onNotificationClick={(reviewId) => {
                  navigate('/dashboard');
                  window.dispatchEvent(new CustomEvent('highlight-review', { detail: reviewId }));
                }}
              />
            )}
