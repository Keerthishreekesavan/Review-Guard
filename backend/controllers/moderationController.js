...
const approveReview = async (req, res) => {
  console.log('[Moderation] Approve request received for:', req.params.id);
  try {
    ...
    const targetUserId = review.userId._id ? review.userId._id.toString() : review.userId.toString();
    const userRoom = `user:${targetUserId}`;
    console.log(`[Socket] Emitting approval to room: ${userRoom}`);
    
    io.to(userRoom).emit('review:status-updated', {
      reviewId: review._id,
      status: 'approved',
    ...
    io.emit('system:threat-log', {
      type: 'cleared',
      message: `✓ MODERATOR APPROVED: Review for "${review.productName}" cleared by ${req.user.name}.`,
...
const rejectReview = async (req, res) => {
  console.log('[Moderation] Reject request received for:', req.params.id);
  try {
    ...
    const targetUserId = review.userId._id ? review.userId._id.toString() : review.userId.toString();
    const userRoom = `user:${targetUserId}`;
    console.log(`[Socket] Emitting rejection to room: ${userRoom}`);
    ...
    io.emit('system:threat-log', {
      type: 'flagged',
      message: `⚠ MODERATOR REJECTED: Review for "${review.productName}" rejected by ${req.user.name}. Reason: ${review.rejectionReason}`,
      timestamp: new Date().toLocaleTimeString('en-US', { hour12: false })
    });
