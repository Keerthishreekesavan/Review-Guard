...
function startAIWorker(io) {
  const worker = new Worker(
    ...
    async (job) => {
      ...
      // --- Emit to Live AI Threat Feed (Admin) ---
      io.emit('system:threat-log', {
        type: toxicityResult.isToxic ? 'flagged' : 'cleared',
        message: toxicityResult.isToxic 
          ? `⚠ AI FLAGGED (Worker): Toxicity ${toxicityResult.score} detected in review #${reviewId}`
          : `✓ AI CLEARED (Worker): Review #${reviewId} passed with toxicity ${toxicityResult.score}`,
